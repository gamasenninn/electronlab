...
test.describe("Generate documentation screenshots", () => {
  ...
  test("#18 Terminal", async () => {
    ...
    // Type a command for a more interesting screenshot
    await terminalPage.waitForTimeout(2000);
    await terminalPage.evaluate(() => {
      window.electronAPI.terminalInput("echo Hello from Electron Lab Terminal!\r");
    });
    ...
    await terminalPage.waitForFunction(
      () => {
        const el = document.querySelector(".xterm-rows");
        return el && el.textContent.includes("Hello from Electron Lab Terminal!");
      },
      { timeout: 15000 }
    );
    ...
    expect(fs.existsSync(path.join(SCREENSHOTS_DIR, "terminal.png"))).toBe(true);

    // Close terminal window
    await electronApp.evaluate(({ BrowserWindow }) => {
      BrowserWindow.getAllWindows().forEach((w) => {
        if (w.webContents.getURL().includes("terminal.html")) w.close();
      });
    });
  });
...
`;
    ...
    expect(content).toContain("terminal.png");
  });
});
